...
const carritoReducer = (state, action) => {
    ...
    if (String(action.payload.id).startsWith("servicio-")) {
        const yaExiste = state.find(item => item.id === action.payload.id)
...
function CarritoProvider({children}) {
    ...
    return(
        <CarritoContext.Provider value={{agregarProducto, eliminarProducto, vaciarCarrito, abierto, setAbierto, abrirCarrito, carrito}}>
            {children}
